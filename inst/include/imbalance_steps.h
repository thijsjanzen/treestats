...
std::vector<int> get_daughters(const ltable& ltab, int parent, int focal_index) {
  std::vector<int> out;
...
std::vector<int> find_daughters(const ltable& ltab, int current_label, int focal_index) {
  std::vector<int> out;
...
std::vector<int> find_others(const ltable& ltab, int current_label, int focal_index) {
  std::vector<int> out;
...
void swap_deepest(ltable& ltab, int& main_attractor, bool& stop) {
  ...
  int max_depth_lineage = 1 + std::distance(depths.begin(),
...
namespace imbal_steps {
  ...
  void rebase_ltable(ltable& ltab) {
    ...
    size_t cnt = 0;
    while(!stop) {
      swap_deepest(ltab, current_main_attractor, stop);
      prev_main_attractor[cnt % prev_main_attractor.size()] = current_main_attractor;
      cnt++;
