context("mean nearest taxon distance")

test_that("usage", {
  if (requireNamespace("picante")) {
    a1 <- treestats::mntd(extant_tree)

    n <- length(extant_tree$tip.label)
    sample_mat <- matrix(data = 1, nrow = n, ncol = n)
    colnames(sample_mat) <- extant_tree$tip.label

    a2 <- picante::mntd(sample_mat, cophenetic(extant_tree),
                        abundance.weighted = FALSE)[[1]]
    testthat::expect_equal(a1, a2)

    ltab <- treestats::phylo_to_l(extant_tree)
    testthat::expect_equal(treestats::mntd(extant_tree),
                           treestats::mntd(ltab))
  }

  focal_ltab <- treestats::phylo_to_l(complete_tree)
  a1 <- treestats::mntd(focal_ltab)
  a2 <- treestats::mntd(complete_tree)
  testthat::expect_equal(a1, a2)
})

test_that("polytomies", {
  if (requireNamespace("picante")) {

    test_func <- function(phy) {
      n <- length(phy$tip.label)
      sample_mat <- matrix(data = 1, nrow = n, ncol = n)
      colnames(sample_mat) <- phy$tip.label

      picante::mntd(sample_mat, cophenetic(phy),
                          abundance.weighted = FALSE)[[1]]
    }

    test_polytomies(treestats::mntd, test_func)
  }
})

test_that("unrooted", {
  set.seed(42)
  focal_tree <- ape::rphylo(n = 100, birth = 1, death = 0)
  a1 <- treestats::mntd(focal_tree)
  a2 <- treestats::mntd(ape::unroot(focal_tree))
  testthat::expect_equal(a1, a2)
})

test_that("wrong_object", {
  testthat::expect_error(
    treestats::mntd(10),
    "input object has to be phylo or ltable"
  )

  testthat::expect_error(
    treestats::mntd(list()),
    "input object has to be phylo or ltable"
  )
})
