...
test_that("usage", {
  ...
  nltt <- treestats::nLTT(tree1, tree1)
  nltt_check <- nLTT::nLTTstat(tree1, tree1)
  ...
  testthat::expect_equal(nltt_base, nltt_base2)
  testthat::expect_equal(nltt_base, nltt_base3)

  empty_tree <- phytools::read.newick(text = "(1:4,2:4):0;")
  nltt_stat <- nLTT::nltt_diff(tree1, empty_tree)
  testthat::expect_equal(nltt_stat, nltt_base)
  testthat::expect_equal(nltt_stat, nltt_base2)
})
