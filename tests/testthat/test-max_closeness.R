...
test_that("usage", {
  a1_1 <- treestats::max_closeness(extant_tree, weight = TRUE)
  a1_2 <- treestats::max_closeness(extant_tree, weight = FALSE)
  ...
  testthat::expect_equal(treestats::max_closeness(extant_tree, weight = TRUE),
                         treestats::max_closeness(ltab, weight = TRUE))

  a1_1 <- treestats::max_closeness(complete_tree, weight = TRUE)
  a1_2 <- treestats::max_closeness(complete_tree, weight = FALSE)
  # because treeCentrality is not available on CRAN, we precompute reference
  # values:
  ...
  testthat::expect_equal(treestats::max_closeness(complete_tree, weight = TRUE),
                         treestats::max_closeness(ltab,       weight = TRUE))
  ...
  if (requireNamespace("igraph")) {
   df <- as.data.frame(cbind(extant_tree$edge,
    ...
    a1 <- treestats::max_closeness(extant_tree)
