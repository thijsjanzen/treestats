...
To test whether outliers are perhaps instead of identifying unique information, 
picking up on random noise, we can also add an extra statistic that has no
meaning, and that has random correlations with all other statistics:
```{r add random}
cor.dist2 <- cor.dist
rand_values <- runif(n = nrow(cor.dist2))
cor.dist2 <- cbind(cor.dist2, rand_values)
cor.dist2 <- rbind(cor.dist2, c(rand_values, NA))
