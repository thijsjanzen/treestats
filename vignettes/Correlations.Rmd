...
Then, we calculate the pearson rank correlation.

```{r pearson}
res.cor <- stats::cor(df, method = "pearson")
res.cor2 <- res.cor # store for later use
```

Statistics are known to correlate with tree size, so we correct for this effect 
by calculating the residuals of a linear regression of each statistic against 
tree size, and then calculating the correlations between these residuals.
